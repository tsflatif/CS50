...
//define line tokens for easy access
#define METHOD          0
...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            {
                error(405); // reply Method Not Allowed
                continue;
            }
            
            if (requestLine[REQUEST_TARGET] == NULL)
            {
                error(400); // reply Bad Request
                continue;
            }
            
            else if (requestLine[REQUEST_TARGET][0] != '/')
            {
                error(501); // reply Not Implemented
                continue;
            }
            ...
            {
                error(505); // reply HTTP Version Not Supported
                continue;
            }
            ...
            char* queryIndex = strchrnul(requestLine[REQUEST_TARGET], '?');
            char* eosIndex = strchr(requestLine[REQUEST_TARGET], '\0');
            char query[(eosIndex - queryIndex < 1) ? 1 : eosIndex - queryIndex];
            
            memcpy(
                query, 
                (strncmp(queryIndex, "\0", 1) == 0) ? queryIndex : queryIndex + 1, 
                (eosIndex - queryIndex < 1) ? 1 : eosIndex - queryIndex
            );

            // TODO: concatenate root and absolute-path
            
            // get absolute path
            queryIndex = strchrnul(requestLine[REQUEST_TARGET], '?');
            char absolutePath[queryIndex - requestLine[REQUEST_TARGET] + 1];
            strncpy(absolutePath, requestLine[REQUEST_TARGET], queryIndex - requestLine[REQUEST_TARGET]);
            
            absolutePath[queryIndex - requestLine[REQUEST_TARGET]] = '\0';
            ...
            char* extIndex = strchr(absolutePath, '.');
            if (extIndex == NULL)
            {
                error(501);
                continue;
            }
            eosIndex = strchr(absolutePath, '\0');
            char extension[eosIndex - extIndex];
            memcpy(extension, extIndex + 1, eosIndex - extIndex);
